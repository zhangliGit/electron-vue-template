      ...
      if (newVal) {
        this.$nextTick(() => {
          ...
          if (lines && lines[this.current_lyric_line]) {
            const top = Number(lines[this.current_lyric_line].offsetTop - LINE_HEIGHT * this.currentLine)
  ...
  methods: {
    handleLineChange(newLine) {
      ...
      if (lines && lines[newLine]) {
        const top = lines[newLine].offsetTop > 0 ? Number(lines[newLine].offsetTop - LINE_HEIGHT * this.currentLine) : 0
